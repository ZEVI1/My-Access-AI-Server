...
namespace MS.Access.MCP.Interop
{
    public class AccessInteropService : IDisposable
    {
            ...
            try
            {
                ...
                if (_accessApplication != null)
                {
                    try
                    {
                        ...
                        catch { /* Expected exception during release of COM objects on shutdown */ }
        ...
        private static void ReleaseComObjectSafe(object? comObject)
        {
            ...
            catch { /* Expected exception during release of COM objects */ }
        }
        ...
        public List<QueryInfo> GetQueries()
        {
            ...
            foreach (System.Data.DataRow row in schema.Rows)
            {
                ...
                {
                    FileLogger.Log($"GetQueries: failed reading query properties: {ex.Message}");
                }
                finally
                {
        ...
        public void LaunchAccess()
        {
            ...
            catch (Exception ex) { FileLogger.Log($"LaunchAccess: failed to set Visible property: {ex.Message}"); }

            try
            {
                accessType.InvokeMember("AutomationSecurity", BindingFlags.SetProperty, null, _accessApplication, new object[] { 3 });
            }
            catch (Exception ex) { FileLogger.Log($"LaunchAccess: failed to set AutomationSecurity property: {ex.Message}"); }

            try
            {
                accessType.InvokeMember("UserControl", BindingFlags.SetProperty, null, _accessApplication, new object[] { false });
            }
            catch (Exception ex) { FileLogger.Log($"LaunchAccess: failed to set UserControl property: {ex.Message}"); }

            try
            {
                accessType.InvokeMember("DisplayAlerts", BindingFlags.SetProperty, null, _accessApplication, new object[] { false });
            }
            catch (Exception ex) { FileLogger.Log($"LaunchAccess: failed to set DisplayAlerts property: {ex.Message}"); }
        ...
        public void CreateForm(string formName)
        {
            ...
            finally
            {
                if (form != null)
                {
                    try { Marshal.ReleaseComObject(form); } catch { /* Expected exception during release of COM objects */ }
                }
            }
        }
        ...
        private FormMetadata ExtractObjectUiMetadata(object accessApp, string objectName, string objectType)
        {
            ...
            finally
            {
                ...
                {
                    FileLogger.Log($"ExtractObjectUiMetadata: failed to close object: {ex.Message}");
                }
            }
        ...
        public List<SystemTableInfo> GetSystemTables()
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    try { created = reader["DateCreate"] != DBNull.Value ? Convert.ToDateTime(reader["DateCreate"]) : DateTime.MinValue; } catch (Exception ex) { FileLogger.Log($"GetSystemTables: failed to parse DateCreate: {ex.Message}"); }
                    try { updated = reader["DateUpdate"] != DBNull.Value ? Convert.ToDateTime(reader["DateUpdate"]) : DateTime.MinValue; } catch (Exception ex) { FileLogger.Log($"GetSystemTables: failed to parse DateUpdate: {ex.Message}"); }
    ...
    public static class FileLogger
    {
        ...
        static FileLogger()
        {
            ...
            catch { /* Ignore logger initialization failures to prevent crashing the host */ }
            ...
            BackgroundWriter = Task.Factory.StartNew(() =>
            {
                foreach (var entry in Queue.GetConsumingEnumerable())
                {
                    ...
                    catch { /* Ignore log write failures to prevent crashing the host */ }
                }
        ...
        public static void Log(object payload)
        {
            ...
            catch { /* Ignore serialization or queue failures to prevent crashing the host */ }
        }
        ...
        public static void Shutdown()
        {
            ...
            catch { /* Ignore shutdown wait failures to prevent crashing the host */ }
        }
    }
